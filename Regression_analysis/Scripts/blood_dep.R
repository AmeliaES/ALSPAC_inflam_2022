...
# ----------------------------------------------------------------------------
# Base model
IL6_nb_nocovars <- glm.nb(dep_episodes ~ IL6_log + Sex,
                 data = dataTrans)

CRP_nb_nocovars <- glm.nb(dep_episodes ~ CRP_log + Sex,
                 data = dataTrans)
# Fully Adjusted
IL6_nb <- glm.nb(dep_episodes ~ IL6_log +
                   Sex + 
                   BMI_age9 +  
                   Maternal.education.at.birth,
                 data = dataTrans)

CRP_nb <- glm.nb(dep_episodes ~ CRP_log +
                   Sex + 
                   BMI_age9 +  
                   Maternal.education.at.birth,
                 data = dataTrans)
...
# Base model
IL6_nb_male_nocovars <- glm.nb(dep_episodes ~ IL6_log,
                          data = dataTransMale)

CRP_nb_male_nocovars <- glm.nb(dep_episodes ~ CRP_log,
                          data = dataTransMale)
# Fully Adjusted
IL6_nb_male <- glm.nb(dep_episodes ~ IL6_log +             
                        BMI_age9 +  
                        Maternal.education.at.birth,
                      data = dataTransMale)

CRP_nb_male <- glm.nb(dep_episodes ~ CRP_log +             
                             BMI_age9 +  
                             Maternal.education.at.birth,
                           data = dataTransMale)
...
# Females
# Base model
IL6_nb_female_nocovars <- glm.nb(dep_episodes ~ IL6_log,
                               data = dataTransFemale)

CRP_nb_female_nocovars <- glm.nb(dep_episodes ~ CRP_log,
                               data = dataTransFemale)
# Fully Adjusted
IL6_nb_female <- glm.nb(dep_episodes ~ IL6_log +             
                        BMI_age9 +  
                        Maternal.education.at.birth,
                      data = dataTransFemale)

CRP_nb_female <- glm.nb(dep_episodes ~ CRP_log +             
                        BMI_age9 +  
                        Maternal.education.at.birth,
                      data = dataTransFemale)
...
resultsBM <- do.call( rbind, 
                   list(result_func(IL6_nb_nocovars, "Base Model", "log(IL-6)"),
                        result_func(CRP_nb_nocovars, "Base Model", "log(CRP)")))
resultsBM
write.csv(resultsBM, "Regression_analysis/Output/blood_dep_base.csv", row.names = F)

resultsFA <- do.call( rbind, 
                    list(result_func(IL6_nb, "Fully Adjusted", "log(IL-6)"),
                         result_func(CRP_nb, "Fully Adjusted", "log(CRP)")))
resultsFA
write.csv(resultsFA, "Regression_analysis/Output/blood_dep_fully_adjusted.csv", row.names = F)
...
# Base model
IL6_nb_male_nocovars <- glm.nb(dep_episodes ~ IL6_log, data = dataTransMale)
CRP_nb_male_nocovars <- glm.nb(dep_episodes ~ CRP_log, data = dataTransMale)
# Adjusted
CRP_nb_male <- glm.nb(dep_episodes ~ CRP_log  + BMI_age9 + Maternal.education.at.birth,
                      data = dataTransMale)
IL6_nb_male <- glm.nb(dep_episodes ~ IL6_log +  BMI_age9 +   Maternal.education.at.birth,
                      data = dataTransMale)

# Females
# Base model
IL6_nb_female_nocovars <- glm.nb(dep_episodes ~ IL6_log,data = dataTransFemale)

CRP_nb_female_nocovars <- glm.nb(dep_episodes ~ CRP_log,  data = dataTransFemale)
# Adjusted
CRP_nb_female <- glm.nb(dep_episodes ~ CRP_log  + BMI_age9 + Maternal.education.at.birth,
                        data = dataTransFemale)
IL6_nb_female <- glm.nb(dep_episodes ~ IL6_log  +  BMI_age9 +   Maternal.education.at.birth,
                        data = dataTransFemale)
...
results_sex
write.csv(results_sex, "Regression_analysis/Output/blood_dep_sex_split_base.csv", row.names = F)
...
results_sex
write.csv(results_sex, "Regression_analysis/Output/blood_dep_sex_split_fully_adjusted.csv", row.names = F)
